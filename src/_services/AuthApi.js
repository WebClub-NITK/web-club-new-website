...
class AuthApi {
	async login(data) {
		...
		var headers = {
			...
			"Access-Control-Allow-Origin": "https://webclub.nitk.ac.in",
		};
		await axios
			.post(
				UrlApi.getBackend() + "/googlelogin",
				{
					token: data.token,
				},
				{ headers: headers }
	...
	async validateToken() {
		...
		if (token === null || token === undefined) {
			...
			await axios
				.post(UrlApi.getBackend() + "/googlelogin", { token: token })
				.then((res) => {
					if (res.status === 200) {
						login = true;
						localStorage.setItem("user_email", res.data);
